...
def show_predict_page():
    ...
    if revenue != 0:
        ...
        if len(lost_growth) != 0:
            fig_3 = go.Bar(x=lost_growth[0], y=lost_growth.index, marker_color='indianred') #px.bar(lost_growth, y=0, x=lost_growth.index)
        ...
        if len(lost_oper.append(lost_growth)) != 0:
            ...
            fig_4.update_layout(title = "Разбивка общей дельты", width=900, height=900)
