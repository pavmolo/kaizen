...
def show_predict_page():
    ...
    if ok:
        ...
        st.markdown(f'Предварительная оценка разницы в прибыли при сравнении с компаниями, реализующими Kaizen: <b>{lost[0]}</b> млн. руб. <p> в том числе: <p>Прибыль упущенная в операционной деятельности: <b>{lost[1]}</b> млн. руб.<p> Прибыль упущенная из-за отсутствия роста: <b>{lost[2]}</b> млн. руб.', unsafe_allow_html=True)
